...
class OpenAPISchema(BaseModel):
    ...
    def validate_paths(cls, paths: dict[str, dict]) -> dict[str, dict]:
        ...
        # 2. 提取paths里的每一个元素，并获取元素下的get/post方法
        methods = ["get", "post"]
        interfaces = []
        extra_paths = {}
        ...
        for interface in interfaces:
            ...
            for parameter in interface["operation"].get("parameters", []):
                ...
                if (
                    ...
                    raise ValidationException(f"parameter.in参数必须为{'/'.join([item.value for item in ParameterIn])}")

                print(parameter.get("type"), "++++++")
