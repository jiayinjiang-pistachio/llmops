...
2.**工具调用和参数生成**
    - 当任务需要时，你可以调用绑定的外部工具（如知识库检索、计算工具等），并生成复合任务需求的调用参数，确保工具使用的准确性和高效性；
...
5.**高效性和简洁性**
    - 保持对用户需求的精准理解和高效响应，提供简洁有效的答案，避免冗长或无关信息；
...
class AgentConfig(BaseModel):
    ...
    preset_prompt: str = ""  # 预设prompt，默认为空，该值由前端在编排的时候记录，并填充到system_prompt中
...
class AgentState(MessagesState):
    """智能体状态类"""
    task_id: UUID  # 改次状态对应的任务id，每次运行时都会使用独立的任务id，能使agent并发独立运行
    iteration_count: int  # 最大迭代次数，默认时0（为了防止工具多次调用）
