...
class AgentQueueManager:
    """智能体队列管理器"""
    # q: Queue 队列改成了键值对的形式，键是task_id，值是对应的队列信息
    user_id: UUID  # 由哪个账号创建的
    ...
    def listen(self, task_id: UUID) -> Generator:
        """监听队列返回的生成式数据"""
        # 1. 记录基础数据记录超时时间、开始时间，最后一次ping通时间
        listen_timeout = 600
    ...
    def publish(self, task_id: UUID, agent_thought: AgentThought) -> None:
        ...
        # 2. 检测事件类型是否为需要停止的类型，涵盖STOP、TIMEOUT、AGENT_END、ERROR
        if agent_thought.event in [QueueEvent.STOP, QueueEvent.ERROR, QueueEvent.AGENT_END, QueueEvent.TIMEOUT]:
            self.stop_listen(task_id)

    def publish_error(self, task_id, error) -> None:
        """发布错误信息到队列"""
        self.publish(task_id, AgentThought(
            id=uuid.uuid4(),
            task_id=task_id,
    ...
    def generate_task_belong_cache_key(cls, task_id: UUID) -> str:
        """生成任务专属的缓存键"""
        return f"generate_task_belong_{str(task_id)}"

    @classmethod
    def generate_task_stopped_cache_key(cls, task_id: UUID) -> str:
        """生成任务已停止的缓存键"""
        return f"generate_task_stopped_{str(task_id)}"
