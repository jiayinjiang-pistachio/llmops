    ...
    def web_app_chat(self, token: str, req: WebAppChatReq, account: Account) -> Generator:
        ...
        if req.conversation_id.data:
            ...
            if (
                    ...
                    or conversation.is_delete is True
            ):
                raise ForbiddenException("该会话不存在，或者不属于当前应用/用户/调试方式")
        else:
            # 如果没传递conversation_id，表示新会话，这时需创建一个会话
            conversation: Conversation = self.create(
                Conversation, **{
                    "app_id": app.id,
