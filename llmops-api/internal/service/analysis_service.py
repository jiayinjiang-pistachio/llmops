...
class AnalysisService(BaseService):
    ...
    def get_app_analysis(self, app_id: UUID, account: Account) -> dict[str, Any]:
        ...
        # 7. 查询消息表最近7天，以及最近14-7天的数据（数据要求答案非空、并且只要计算的数据）
        ...
        # 11. 定义5个指标字段名称
        fields = [
            "total_messages", "active_accounts", "avg_of_conversation_messages", "token_output_rate", "cost_consumption"
        ]
    ...
    def get_messages_by_time_range(self, app: App, start_at: datetime, end_at: datetime):
    ...
    @classmethod
    def calculate_overview_indicators_by_messages(cls, messages: list[Message]):
        """根据传递的消息列表计算概览指标：涵盖全部会话数、激活用户数、平均互动数、Token输出速度、费用消耗"""
        # 计算全部会话数，使用消息总数计算
        ...
        return {
            ...
            "avg_of_conversation_messages": avg_of_conversation_messages,
            "token_output_rate": float(token_output_rate),
            "cost_consumption": float(cost_consumption),
        }

    @classmethod
    def calculate_pop_by_overview_indicators(cls, current_data: dict[str, Any], previous_data: dict[str, Any]) -> dict[
        str, Any]:
    ...
    @classmethod
    def calculate_trend_by_messages(cls, end_at: datetime, days_ago: int, messages: list[Message]):
        """根据传递的结束时间、回退天数、消息列表计算指标的趋势数据"""
        # 1. 重新计算end_at为午夜时间
