...
from internal.model import App, ApiTool, Dataset, AppConfigVersion, AppConfig, AppDatasetJoin, Workflow
...
from ..entity.app_entity import DEFAULT_APP_CONFIG
...
class AppConfigService(BaseService):
    ...
    def get_draft_app_config(self, app: App) -> dict[str, Any]:
        ...
        # 2. 校验model_config信息，如果使用了不存在的提供者或模型，则使用默认值（宽松校验）
        ...
        # 8. 将数据转换成字典返回
        return self._process_and_transform_app_config(
    ...
    def get_app_config(self, app: App) -> dict[str, Any]:
        ...
        # 2. 校验model_config，如果运行时配置的model_config发生变化，则进行更新
        ...
        if app_config.model_config != validate_model_config:
            print("---validate_model_config---", validate_model_config)
    ...
    def get_langchian_tools_by_tool_config(self, tools_config: list[dict]) -> list[BaseTool]:
        """根据传递的工具配置获取langchain工具列表"""
        tools = []
        for tool in tools_config:
            # 1. 根据不同的工具类型执行不同的操作
            if tool["type"] == "builtin_tool":
                # 2. 内置工具，通过builtin_provider_manager获取工具实例
                ...
                # 3. API工具，首先根据id找到ApiTool记录，然后创建实例
                api_tool: ApiTool = self.get(ApiTool, tool["tool"]["id"])
                if not api_tool:
                    continue
                tools.append(
    ...
    def get_langchian_tools_by_workflow_ids(self, workflow_ids: list[UUID]) -> list[BaseTool]:
        """根据传递的工作流列表获取langchain工具列表"""
        # 查询工作流记录信息
    ...
    def _process_and_validate_tools(self, origin_tools: list[dict]) -> tuple[list[dict], list[dict]]:
        ...
        for tool in origin_tools:
            ...
            if tool["type"] == "builtin_tool":
                ...
                # 3. 判断工具的params与草稿配置中的params是否一致，不一致则设置为默认值
                ...
                # 4. 数据都存在，并且参数已经校验完毕，可以将数据添加到validate_tools
    ...
    def _process_and_validate_datasets(self, origin_datasets: list[UUID]) -> tuple[list[dict], list[UUID]]:
        """根据传递的原始知识库信息进行处理和校验"""
        # 1. 校验知识库列表，如果引用了不存在/被删除的知识库，需要剔除数据并更新，同时获取知识库额外信息
        datasets = []
        # 2. 根据origin_datasets中的知识库id列表查询数据库中的知识库记录
        ...
        # 3. 计算存在的知识库id列表，为了保留原始顺序，使用列表循环的方式来判断
    ...
    def _process_and_transform_app_config(
        ...
        """根据新的工具信息、工作流信息、知识库信息，获取转换后的应用配置"""
        print("----_process_and_transform_app_config.model_config----", model_config)
    ...
    def _process_and_validate_model_config(self, origin_model_config: dict[str, Any]) -> dict[str, Any]:
        ...
        print("---origin_model_config---", origin_model_config)
        # 1. 判断model_config是否为字典
        if not isinstance(origin_model_config, dict):
            return DEFAULT_APP_CONFIG["model_config"]

        # 2. 提取origin_model_config中的provider、model、parameters
        model_config = {
        ...
        print("---model_entity---", model_entity)
    ...
    def _process_and_validate_workflows(self, origin_workflows: list[UUID]) -> tuple[list[dict], list[UUID]]:
        ...
        # 1. 校验工作流列表，如果引入了不存在/被删除的工作流，则需要更新数据，同时获取工作流额外信息
        workflows = []
        workflow_records: list[Workflow] = self.db.session.query(Workflow).filter(
