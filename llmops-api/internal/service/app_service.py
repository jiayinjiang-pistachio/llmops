...
class AppService(BaseService):
    ...
    def auto_create_app(self, name: str, description: str, account_id: UUID):
        ...
        account: Account = self.db.session.query(Account).get(account_id)
    ...
    def debug_chat(self, app_id: UUID, query: str, account: Account) -> Generator:
        ...
        tools = self.app_config_service.get_langchian_tools_by_tool_config(draft_app_config["tools"])
        ...
        for agent_thought in agent.stream({
            ...
            # 11. 提取 thought和answer
            event_id = str(agent_thought.id)
            task_id = str(agent_thought.task_id)

            # 12. 将数据填充到agent_thought，便于存储到数据库中
            if agent_thought.event != QueueEvent.PING:
                # 13. 除了 agent_message 叠加，其他事件一次性返回
