...
class AppService(BaseService):
    ...
    def _validate_draft_app_config(self, draft_app_config: dict[str, Any], account: Account) -> dict[str, Any]:
        ...
        if "preset_prompt" in draft_app_config:
            ...
            if not isinstance(preset_prompt, str) or len(preset_prompt) > 2000:
                raise ValidationException("人设与恢复逻辑必须是字符串，长度在0-2000个字符")
        ...
        if "tools" in draft_app_config:
            ...
            # 6.1 tools 类型必须为列表，空列表则代表不绑定任何工具
            if not isinstance(tools, list):
                raise ValidationException("工具列表必须是列表类型数据")
        ...
        if "retrieval_config" in draft_app_config:
            ...
            if retrieval_config["retrieval_strategy"] not in ["semantic", "full_text", "hybrid"]:
                raise ValidationException("检索策略格式错误")
    ...
    def fallback_history_to_draft(self, app_id: UUID, app_config_version_id: UUID,
        ...
        """根据传递的应用id+历史配置版本idd、账号信息，回退特定配置到草稿"""
        # 1. 校验应用权限并获取信息
        app = self.get_app(app_id, account)
        ...
        # 4. 校验历史版本配置信息
        draft_app_config_copy = self._validate_draft_app_config(draft_app_config_copy, account)
    ...
    def get_debug_conversation_summary(self, app_id: UUID, account: Account) -> str:
        ...
        if draft_app_config["long_term_memory"]["enable"] is False:
            raise FailException("该应用并未开启长期记忆哦，无法获取")
    ...
    def delete_debug_conversation(self, app_id: UUID, account: Account) -> App:
        ...
        # 2. 判断是否存在debug_conversation_id这个数据，如果不存在标识没有会话，无需执行任何操作
        if not app.debug_conversation.id:
            return app

        # 3. 否则将debug_conversation_id 的值重置为None
    ...
    def debug_chat(self, app_id: UUID, query: str, account: Account) -> Generator:
        ...
        # 5. 检测是否开启输入审核，并检测输入，如果存在敏感词则使用预设响应并结束代码
        ...
        # todo: 6. 根据传递的model_config实例化不同的LLM模型，等待多LLM接入后该处会发生变化
        llm = ChatOpenAI(
        ...
        for tool in draft_app_config["tools"]:
            ...
            if tool["type"] == "builtin_tool":
                ...
                # 11. API工具，首先根据id找到ApiTool记录，然后创建实例
        ...
        if draft_app_config["datasets"]:
            ...
            dataset_retrieval = self.retrieval_service.create_langchain_tool_from_search(
                flask_app=current_app._get_current_object(),  # <--- 关键点, 使用 _get_current_object() 获取真实的 Flask 实例
        ...
        # 14. todo: 构建Agent智能体，目前暂时使用FunctionCallAgent
        ...
        for agent_queue_event in agent.run(query, history, debug_conversation.summary):
            ...
            # 17. 将数据填充到agent_thought，便于存储到数据库中
            if agent_queue_event.event != QueueEvent.PING:
                # 18. 除了 agent_message 叠加，其他事件一次性返回
