...
class DocumentService(BaseService):
    ...
    def create_documents(
        ...
        if len(upload_files) == 0:
            logging.warning(
                f"上传文件列表未解析到合法文件，account_id: {account.id}, dataset_id: {dataset_id}, upload_file_ids: {upload_file_ids}"
            )
    ...
    def update_document(self, dataset_id, document_id, account: Account, **kwargs):
    ...
    def get_documents_with_page(self, dataset_id: UUID, account: Account, req: GetDocumentsWithPageReq):
    ...
    def update_document_enabled(self, dataset_id: UUID, document_id: UUID, enabled: bool, account: Account):
