...
from sqlalchemy import (PrimaryKeyConstraint, Index, Column, UUID, text, String, DateTime, Boolean)
...
class WechatConfig(db.Model):
    ...
    wechat_app_secret = Column(String(255), nullable=True, server_default=text("''::character varying"))  # 微信公众号开发者密钥
...
class WechatEndUser(db.Model):
    ...
    id = Column(UUID, nullable=False, server_default=text("uuid_generate_v4()"))
    openid = Column(String, nullable=False)
    app_id = Column(UUID, nullable=False)
    end_user_id = Column(UUID, nullable=False)
    updated_at = Column(
        DateTime,
        nullable=False,
        server_default=text("CURRENT_TIMESTAMP(0)"),
...
class WechatMessage(db.Model):
    ...
    id = Column(UUID, nullable=False, server_default=text("uuid_generate_v4()"))
    wechat_end_user_id = Column(UUID, nullable=False)
    message_id = Column(UUID, nullable=False)
    is_pushed = Column(Boolean, nullable=False, server_default=text("false"))  # 是否推送，默认false表示未推送
    updated_at = Column(
        DateTime,
        nullable=False,
        server_default=text("CURRENT_TIMESTAMP(0)"),
