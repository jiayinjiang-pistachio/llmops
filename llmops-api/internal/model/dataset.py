...
# -*- coding: utf-8 -*-
"""
@Time           : 2025/12/31 16:54
@Author         : jiayinkong@163.com
@File           : dataset.py
@Description    : 
"""
from sqlalchemy import PrimaryKeyConstraint, Column, UUID, text, String, Text, DateTime, func, Integer, Boolean
...
class Dataset(db.Model):
    ...
    def related_app_count(self) -> int:
        """只读属性，获取该知识库关联的应用数"""
        return (
            db.session.query(func.count(AppDatasetJoin.id)).filter(
    ...
    def character_count(self) -> int:
        """只读属性，获取该知识库下的字符总数"""
        return db.session.query(func.coalesce(func.sum(Document.character_count), 0)).filter(
