...
const workflows = defineModel<DraftAppConfig['workflows']>('workflows', {
  required: true,
  default: () => [],
})
...
const activateWorkflows = ref<DraftAppConfig['workflows']>([])
const originWorkflows = ref<DraftAppConfig['workflows']>([])
...
const handleSubmitWorkflows = async () => {
  try {
    ...
    await handleUpdateDraftAppConfig(props.app_id, {
      workflows: activateWorkflows.value.map((activeWorkflow) => activeWorkflow.id),
    })
    ...
    // 8.3 双向同步更新props中的数据
    workflows.value = activateWorkflows.value
  ...
  (newValue) => {
    ...
    if (!isWorkflowsInit.value || !isWorkflowsModified()) {
      ...
      if (newValue && newValue.length > 0) {
        // 10.3 赋初始值
        const initData = workflows.value.map((workflow) => {
          return {
            id: workflow.id,
            name: workflow.name,
            icon: workflow.icon,
...
const handleClickDeleteWorkflow = async (idx: number) => {
  // 1.清除props中指定的数据
  const newWorkflows = [...workflows.value]
  ...
  // 通知父组件，数据发生变化，相当于 emits update:workflows
...
<template>
  <div class="">
    <a-collapse-item key="workflows" class="app-ability-item">
      ...
      <div v-if="workflows?.length > 0" class="flex flex-col gap-1">
        <div
        ...
        >
          ...
          <a-button
            ...
            @click="() => handleClickDeleteWorkflow(idx)"
          >
            <template #icon>
              <icon-delete />
            </template>
    ...
    >
      ...
      <div class="flex items-center justify-between mb-6">
        <div class="text-lg font-bold text-gray-700">选择引用工作流</div>
        <a-button
          type="text"
          class="!text-gray-700"
          size="small"
        ...
        >
          ...
          <div class="flex flex-col gap-2">
            ...
            <div
              ...
              :key="workflow.id"
              :class="`flex items-center gap-2 border px-3 py-2 rounded-lg cursor-pointer hover:bg-blue-50 hover:border-blue-700 ${activateWorkflows.some((activeWorkflow) => activeWorkflow.id === workflow.id) ? 'bg-blue-50 border-blue-700' : ''}`"
