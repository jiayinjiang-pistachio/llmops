...
const props = defineProps<{
  app_id: string
  draft_app_config: DraftAppConfig
}>()
defineEmits(['update:draft_app_config'])
...
const dynamicDraftAppConfig = computed(() => props.draft_app_config)
</script>
    ...
    <div class="flex-1 overflow-scroll scrollbar-w-none">
      <a-collapse :bordered="false" :default-active-key="defaultActivateKeys">
        ...
        <long-term-memory-ability-item
          v-model:long_term_memory="dynamicDraftAppConfig.long_term_memory"
          :app_id="app_id"
        />
        <!-- 对话开场白 -->
        <opening-ability-item
          v-model:opening_questions="dynamicDraftAppConfig.opening_questions"
          v-model:opening_statement="dynamicDraftAppConfig.opening_statement"
          :app_id="app_id"
        />
        <!-- 回答后生成建议问题 -->
