...
const props = defineProps({
  app: { type: Object as PropType<AppDetail>, default: () => ({}), required: true },
...
const handleSubmit = async () => {
  ...
  await handleDebugChat(props.app?.id, humanQuery, (event_response) => {
    ...
    const event_id: string = data?.id
    const agent_thoughts: DebugConversationMessageItem['agent_thoughts'] =
      messages.value[0]!.agent_thoughts

    // 5.8 初始化数据检测与赋值
    if (message_id.value === '' && data?.message_id && messages.value[0]) {
      ...
      if (event === QueueEvent.agentMessage) {
        ...
        if (agent_thought_idx === -1) {
          ...
          agent_thoughts[agent_thought_idx] = {
            ...
            latency: data?.latency,
          } as DebugConversationMessageItem['agent_thoughts'][0]
        }
  ...
  <div class="">
    ...
    <div v-else class="flex flex-col p-6 gap-2 items-center justify-center h-[calc(100vh-238px)]">
      ...
      <div class="flex flex-col items-center gap-2">
        <a-avatar :size="48" shape="square" class="rounded-lg" :image-url="app.icon" />
        ...
        <a-button
          ...
          @click="
            async () => {
              ...
              // 2.调用api接口清空会话
              await handleDeleteDebugConversation(app.id)

              // 3.重新获取数据
              await loadDebugConversationMessages(app.id, true)
            }
          "
        >
          <template #icon>
