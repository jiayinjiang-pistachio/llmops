...
const handleScroll = async (event: UIEvent) => {
  ...
  if (scrollTop + clientHeight >= scrollHeight - 10) {
    ...
    await loadWorkflows(String(route.query?.search ?? ''), '')
  }
}
...
onMounted(async () => {
  // 初始化工作流数据数据
  await loadWorkflows(String(route.query?.search ?? ''), '', true)
})
...
watch(
  () => route.query?.search,
  async () => await loadWorkflows(String(route.query?.search ?? ''), '', true),
)
</script>
...
<template>
  ...
  >
    ...
    <create-or-update-workflow-modal
      ...
      :callback="async () => await loadWorkflows(String(route.query?.search ?? ''), '', true)"
    />
  </a-spin>
</template>
