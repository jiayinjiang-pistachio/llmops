...
const latency = computed(() => {
  return nodeResults.value.reduce((total, item) => total + (item.latency || 0), 0)
})

// 定义插件的耗时
const toolLatency = computed(() => {
  return nodeResults.value
    .filter((item) => item.node_data.node_type === 'tool')
    .reduce((total, item) => total + (item.latency || 0), 0)
})
...
const onSubmit = async ({ errors }: { errors: Record<string, ValidatedError | undefined> }) => {
  ...
  // 调用调试工作流的hooks
  await handleDebugWorkflow(props.workflowId, form.value, (eventResponse) => {
    nodeResults.value.push(eventResponse.data)
  })
}
