...
watch(
  ...
  (newValue) => {
    if (
      isEmpty(newValue) ||
      (newValue.provider === form.value.provider && newValue.model === form.value.model)
    ) {
      return
    }
    ...
    if (newValue?.provider) {
      loadLanguageModel(String(newValue?.provider), String(newValue?.model))
    }
  },
  { immediate: true },
)
