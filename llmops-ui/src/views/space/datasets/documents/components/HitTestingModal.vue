<template>
          ...
          <div class="border border-blue-700 bg-blue-100 rounded-lg flex flex-col mb-6">
            ...
            <div class="flex items-center justify-between px-4 py-1.5">
              ...
              >
                ...
                <div v-if="hitTestingForm.retrieval_strategy === 'semantic'">相似性检索</div>
                <div v-if="hitTestingForm.retrieval_strategy === 'full_text'">全文检索</div>
                <div v-else>混合检索</div>
              </a-button>
            </div>
            <!-- 输入框容器 -->
  ...
  >
    ...
    <!-- 中间表单内容 -->
    <a-form :model="retrievalSettingForm" class="pt-6" @submit="saveRetrievalSetting">
      ...
      <a-form-item field="k" label="最大召回数量">
        <div class="flex items-center gap-4 w-full pl-3">
          <a-slider v-model="retrievalSettingForm.k" :step="1" :min="1" :max="10" />
          <a-input-number
...
import type { DatasetQueryItem, HitItem } from '@/models/dataset'
import { getDatasetQueries, hit } from '@/services/datasets'
...
const queires = reactive<DatasetQueryItem[]>([])
const loadQueries = async () => {
  try {
    ...
    queires.splice(0, queires.length, ...data)
  } finally {
...
const hitTestingSegments = reactive<HitItem[]>([])
...
watch(
  ...
  async (newValue) => {
    if (newValue) {
      ...
      queires.splice(0, queires.length)
      hitTestingSegments.splice(0, hitTestingSegments.length)
