<template>
  ...
  <div class="px-6 pt-6 flex flex-col overflow-hidden h-full">
    ...
    <div class="sticky top-0 z-20 bg-gray-50">
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <a-space :size="12">
          ...
          >
            <div v-if="document.enabled" class="w-2 h-2 bg-green-500 rounded-sm"></div>
              ...
              <div class="flex items-center">
                <div class="flex items-center gap-1 text-xs text-gray-700">
                  ...
                  <div
                    ...
                    class="w-2 h-2 bg-green-500 border border-green-500 rounded-sm"
                  ></div>
                  <div v-else class="w-2 h-2 bg-gray-500 border border-gray-500 rounded-sm"></div>
                </div>
                  ...
                  async () => {
                    await handleDelete(
                      ...
                      () => {
                        loadSegments(true)
                        loadDocument(
                          route.params.dataset_id ? String(route.params.dataset_id) : '',
                          route.params.document_id ? String(route.params.document_id) : '',
                        )
                      },
                    )
                  }
    ...
    <!-- 新建/修改模态窗 -->
    <createOrUpdateSegmentModal
      v-model:visible="createOrUpdateModalVisible"
      :dataset_id="route.params.dataset_id ? String(route.params.dataset_id) : ''"
      :document_id="route.params.document_id ? String(route.params.document_id) : ''"
